					...
					<table cellspacing="0" summary="[프로그램 입력] 글쓰기 입력 양식입니다."
						...
						<tbody>
							...
							<tr>
								...
								<td class="last"><input type="file" id="files<%=(i + 1)%>"
									name="files" size="50" class="atchfile" /></td>
							</tr>
							<%
								} else {
										Hashtable<String, String> map = (Hashtable<String, String>) request.getAttribute("filemap");
							%>
							<tr>
								<th scope="row" class="first"><label for="atchfile">첨부파일</label></th>
								<td class="last"><input type="file" id="files<%=(i + 1)%>"
									name="files" size="50" class="atchfile" /> <input
									type="checkbox" id="del<%=(i + 1)%>"
									onclick="goChange('<%=i + 1%>')" value="<%=(i + 1)%>">삭제
									&nbsp;&nbsp; <%=map.get("filename" + (i + 1))%> <input
									type="hidden" name="old_name" id="old_name"
									value="<%=map.get("filename" + (i + 1))%>"></td>
							</tr>
							<%
								}
								}
...
$(document).ready(function(){
	...
	$("#btnWrite").click(function(){
		...
		*/
		...
		var url="${commonURL}/board/save.mt";
		if('${param.mode}'=='update')
			url="${commonURL}/board/update.mt";
		else if('${param.mode}'=='reply' )
			url="${commonURL}/board/reply.mt";
